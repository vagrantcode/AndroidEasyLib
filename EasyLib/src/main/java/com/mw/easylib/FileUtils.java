...
public class FileUtils {
     ...
     * @length length 文件长度
     */
    public static boolean createFile(String path, String fileName, long length) {
        ...
        if (file.exists()) {
            ...
            try {
                return createFixFile(file, length);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
     ...
     * 主要用于占位
     * */
    private static Boolean createFixFile(File file, long length) throws IOException {
    ...
    public static void setBaseDic(String baseDic) {
        FileUtils.baseDic = baseDic;
    }
    ...
    /**
     * 方法说明：从SD卡上读取TXT文件，失败或文件不存在返回null
     *
     * @param url
     * @return StringBuffer
     */
